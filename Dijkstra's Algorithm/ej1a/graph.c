...
static bool invrep(graph_t g) {
    bool inv = true;
    for (unsigned int i = 0; i < g->max_vertexs && inv ; i++)
    {
      inv = g->costs[g_index(i,i,g->max_vertexs)] == 0; 
    }
    return inv;
}
...
graph_t graph_empty(unsigned int max_vertexs)
{
    ...
    for (vertex_t i = 0; i < max_vertexs; i++)
    {
        costs_array[g_index(i,i,max_vertexs)] = 0;
    }
...
void graph_add_edge(graph_t graph, vertex_t from, vertex_t to, cost_t cost)
{
    ...
    graph->costs[g_index(from ,to ,graph->max_vertexs)] = cost;
    assert(invrep(graph));
}
...
cost_t graph_get_cost(graph_t graph, vertex_t from, vertex_t to)
{
    ...
    assert(from < graph->max_vertexs);
    assert(to < graph->max_vertexs);    
    return graph->costs[g_index(from,to,graph->max_vertexs)];
}
